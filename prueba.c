...
char Dire, Valor,INT_SOURCE,DireH,DireL;
...
////    U1TXREG=Vector[5];
////    while(BusyUART1());
//     datt=IMUBREAD(0x0D);//Leo registro 0x0D rta 0x6A
//    U1TXREG=datt;
//    while(U1STAbits.TRMT);
//    WriteUART1('A');
//	while(while(U1STAbits.TRMT););
////    WriteUART1(0x0d);
////	while(BusyUART1());  
//   }
...
//        U1TXREG= 'A';
//       while(!U1STAbits.TRMT);
//       INT_SOURCE=IMUBREAD(0x0E);//leo INT_SOURCE
//       U1TXREG=INT_SOURCE ;
//       while(!U1STAbits.TRMT);
//       //*******************FIN*********************** 
...
//      
        ...
        case 0xAA:                //case 0xAAmodo prueba
            parar=1;
        while(parar==1)
       {
        VectorRS[0]=0xFE;
...
printf("s");
 ...
 if(orden==0x46||orden==0x48)//Si llega la orden 0x46 o 0x48 deja de enviar o almacenar datos
 {parar=0;
 }
 if (orden==0x50)// llegan 6 bytes por ahora
 {
 while (!DataRdyUART1());
 DireH=U1RXREG;
 while (!DataRdyUART1());
